...
jarvis-core 是一套前后端分离的管理后台基础框架，面向需要快速搭建企业级后台、又希望保留统一对外 API 能力的团队。项目从完整 Admin 体系中精简而来，聚焦两大核心：系统管理与开放平台，去掉 AI、语音、外呼等业务模块，代码结构清晰，适合作为二次开发的起点或直接开源发布。
...
开放平台是 jarvis-core 的差异化能力：对外提供统一网关入口，第三方应用通过 AppID、签名与 3DES 加密调用注册的 Action；管理端支持应用管理、接口同步、在线文档、调用统计与配额控制。仓库附带 Go / Python / Java SDK 示例（Echo 演示），文档中说明了网关集群与调用方集群的 Redis 会话共享、握手加锁等生产实践。

项目包含完整 docs/ 文档（快速开始、架构、API、部署、开发指南）、Docker 后端部署模板，以及 Cursor Skills 等工程化辅助。无论是搭建内部管理系统、Saas 运营后台，还是在统一网关之上扩展行业 Action，jarvis-core 都可以作为稳定、可扩展的基础底座。
