# -*- coding: utf-8 -*-
"""Write jarvis project documentation (UTF-8).
...
jarvis 是一套前后端分离的管理后台基础框架，内置 **系统管理**（用户 / 角色 / 菜单 / 字典）与 **开放平台**（网关、应用、接口文档、调用统计），可作为业务系统的起点快速扩展。
...
jarvis API **多副本/多节点**部署时，**必须**配置共享 **Redis**，否则开放平台会话无法跨节点一致。
...
业务系统若以 **多实例、多 Pod、多容器** 调用 jarvis 开放平台，须自行实现 **集群版客户端**（可参考 `robot-dms-admin` 中 `continew-funcapi` 的 `OpenPlatformClient` 集群实现思路）。
...
> 调用方 Redis 与 jarvis 服务端 Redis **相互独立**；键名前缀建议与业务隔离，避免与 `open:session:{token}`（服务端）混淆。
...
| jarvis 网关多副本 | **必须** 服务端 Redis（见上一节） |
...
演示如何通过统一网关调用 jarvis 开放平台（握手、3DES 加密、Echo 演示接口）。
...
1. jarvis 后端已启动（默认 `http://127.0.0.1:8000`）
